-- ESP Module for ULTIMATE Script (Sirius ESP Integrated)
local ESPModule = {
    Library = nil
}

-- [[ SIRIUS ESP LIBRARY START ]]
local espLibrary = {
    instances = {},
    espCache = {},
    chamsCache = {},
    ...
    options = {
        ...
        tracerOrigin = "Bottom",
        chams = false, -- Disabled by default for performance
    ...
    runService:BindToRenderStep("esp_rendering", renderValue or (Enum.RenderPriority.Camera.Value + 1), function()
        for player, objects in next, self.espCache do
            ...
            if (character and torso) then
                ...
                objects.arrow.PointA = pointA; objects.arrow.PointB = pointB; objects.arrow.PointC = pointC;
                ...
                objects.arrowOutline.PointA = pointA; objects.arrowOutline.PointB = pointB; objects.arrowOutline.PointC = pointC;
                objects.top.Visible = show and self.options.names;
                objects.top.Font = self.options.font; objects.top.Size = self.options.fontSize;
                objects.top.Transparency = self.options.nameTransparency; objects.top.Color = color or self.options.nameColor;
                ...
                objects.side.Font = self.options.font; objects.side.Size = self.options.fontSize;
                objects.side.Transparency = self.options.healthTextTransparency; objects.side.Color = color or self.options.healthTextColor;
                ...
                objects.bottom.Font = self.options.font; objects.bottom.Size = self.options.fontSize;
                objects.bottom.Transparency = self.options.distanceTransparency; objects.bottom.Color = color or self.options.nameColor;
                ...
                objects.box.Size = size; objects.box.Position = position;
                objects.boxOutline.Visible = show and self.options.boxes;
                objects.boxOutline.Transparency = self.options.boxesTransparency;
                objects.boxOutline.Size = size; objects.boxOutline.Position = position;
                ...
                objects.boxFill.Size = size; objects.boxFill.Position = position;
                ...
                objects.healthBar.Size = healthBarSize; objects.healthBar.Position = healthBarPosition;
                ...
                objects.line.From = origin == "Mouse" and userInputService:GetMouseLocation() or origin == "Top" and vector2New(viewportSize.X * 0.5, 0) or origin == "Bottom" and vector2New(viewportSize.X * 0.5, viewportSize.Y);
                objects.line.To = torsoPosition;
            else
                for _, object in next, objects do object.Visible = false; end
            end
        end
...
function espLibrary:Unload()
    for _, connection in next, self.conns do connection:Disconnect(); end
    for _, player in next, players:GetPlayers() do self.removeEsp(player); self.removeChams(player); end
    for _, object in next, self.instances do object:Destroy(); end
    screenGui:Destroy();
    runService:UnbindFromRenderStep("esp_rendering");
end
-- [[ SIRIUS ESP LIBRARY END ]]

-- ESP Module Logic
function ESPModule:Toggle(value)
    if not self.Library then
        self.Library = espLibrary
        self.Library:Load()
    end
    self.Library.options.enabled = value
end

function ESPModule:SetBoxes(value)
    if self.Library then self.Library.options.boxes = value end
end

function ESPModule:SetNames(value)
    if self.Library then self.Library.options.names = value end
end

function ESPModule:SetTracers(value)
    if self.Library then self.Library.options.tracers = value end
end

function ESPModule:SetTeamCheck(value)
    if self.Library then self.Library.options.teamCheck = value end
end
